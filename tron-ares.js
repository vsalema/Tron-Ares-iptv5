...
// =====================================================
// RADIO: STOP SANS RESTORE (quand l’utilisateur choisit une autre chaîne/liste)
// =====================================================
function isRadioOverlayVisible() {
  return !!(radioOverlayLayer && radioOverlayLayer.style.display && radioOverlayLayer.style.display !== 'none');
}

function stopRadioForNavigation() {
  // Stoppe la radio + ferme l’overlay, mais NE relance PAS l’ancienne diffusion
  try { radioAudio?.pause(); } catch {}
  radioPlaying = false;
  if (radioPlayBtn) radioPlayBtn.textContent = '▶';
  miniRadioEl?.classList.remove('playing');
  ...
  // Restaure l’audio vidéo (la nouvelle sélection va ensuite lancer son propre flux)
  try {
    if (videoEl) {
      videoEl.muted = prevVideoMuted;
      videoEl.volume = prevVideoVolume;
...
function playEntryAsOverlay(entry) {
  ...
  if (typeof radioAudio !== 'undefined' && (radioPlaying || isRadioOverlayVisible() || lastPlaybackSnapshot)) {
...
function playUrl(entry) {
  ...
  // Si l’overlay radio est ouvert (même en pause), on le ferme dès qu’on choisit une autre entrée
  if (typeof radioAudio !== 'undefined' && (radioPlaying || isRadioOverlayVisible() || lastPlaybackSnapshot)) {
    stopRadioForNavigation();
  }
