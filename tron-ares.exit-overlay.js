...
// =====================================================
// ✅ EXIT RADIO OVERLAY WHEN USER CLICKS LIST BUTTONS
// (ne casse aucune action existante : on ne bloque pas l’event)
// =====================================================
function exitRadioOverlayForSidebarAction(opts = {}) {
  const stopRadio = !!opts.stopRadio;

  const visible = !!(radioOverlayLayer && radioOverlayLayer.style.display !== 'none');
  if (!visible) return false;

  // Ferme uniquement l’UI overlay radio (sans "restore" automatique du snapshot)
  hideRadioOverlayInPlayer();
  lastPlaybackSnapshot = null;

  // Restaure le son vidéo (car le mode radio force mute/volume=0)
  try {
    if (videoEl) {
  ...
  // Optionnel : couper la radio quand l’action lance une lecture (chaîne / overlay)
  if (stopRadio) {
...
// ✅ Si l’overlay radio est actif : dès qu’on clique sur une action des listes (sidebar),
// on sort de l’overlay sans casser les actions déjà présentes.
document.addEventListener('click', (ev) => {
  // Cible uniquement les zones "listes" / onglets dans la sidebar
  const inSidebarLists = !!ev.target.closest('#channelFrList, #channelList, #iframeList, #favoriteList, .tabs');
  if (!inSidebarLists) return;

  // Si on clique un simple "favoris ★", on ferme l’overlay mais on ne coupe pas la radio.
  const btn = ev.target.closest('button');
  const btnTitle = (btn?.title || '').toLowerCase();

  const isFavToggle = !!(btn && btn.classList.contains('icon-btn') && btnTitle.includes('favoris'));
  const isTabSwitch = !!ev.target.closest('.tab-btn');

  // Si c’est une vraie sélection/lecture (click item/overlay), on coupe la radio.
  const isPlaybackSelection = !!ev.target.closest('.channel-item') && !isFavToggle;

  exitRadioOverlayForSidebarAction({
    stopRadio: isPlaybackSelection && !isTabSwitch
  });
}, true);
