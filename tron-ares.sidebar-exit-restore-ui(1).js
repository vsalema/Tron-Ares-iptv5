...
// -----------------------------------------------------
// Stop radio + close overlay, BUT don't restore snapshot.
// (On laisse les handlers existants lancer la chaîne/overlay sélectionné.)
// -----------------------------------------------------
function stopRadioAndExitOverlayOnly() {
  try { radioAudio?.pause(); } catch {}
  radioPlaying = false;
  if (radioPlayBtn) radioPlayBtn.textContent = '▶';
  miniRadioEl?.classList.remove('playing');

  // On sort juste de l'UI radio
  hideRadioOverlayInPlayer();

  // On remet l'audio vidéo comme avant (sinon la nouvelle sélection peut rester muette)
  try {
    if (videoEl) {
      videoEl.muted = prevVideoMuted;
      videoEl.volume = prevVideoVolume;
  ...
  // On ne veut plus "retour radio" ensuite
  lastPlaybackSnapshot = null;
}

// =====================================================
// EXIT RADIO OVERLAY ON ANY SIDEBAR ACTION
// (clic sur n'importe quel bouton / liste / onglet du menu)
// -> on sort du mode mini-radio et on revient à la chaîne en cours
// =====================================================
function isRadioOverlayVisible() {
  const layer = radioOverlayLayer || document.getElementById('radioOverlayLayer');
  if (!layer) return false;

  // style inline prioritaire
  if (layer.style && layer.style.display) return layer.style.display !== 'none';
...
function exitRadioOverlayOnSidebarClick(ev) {
  const t = ev.target;
  if (!t) return;

  // On ne déclenche que sur les clics qui viennent vraiment des LISTES
  // (pas tout le menu), pour éviter les effets de bord.
  const inLists =
    (channelFrListEl && channelFrListEl.contains(t)) ||
    (channelListEl && channelListEl.contains(t)) ||
    (iframeListEl && iframeListEl.contains(t)) ||
    (favoriteListEl && favoriteListEl.contains(t));

  if (!inLists) return;
  if (!isRadioOverlayVisible()) return;

  // Important : ne bloque pas les handlers existants (pas de preventDefault / stopPropagation)
  // - Si l'action va lancer une lecture (clic item / bouton ⧉), on ne restaure pas le snapshot
  //   (sinon flash / double action). On ferme juste l'overlay + on remet l'audio vidéo.
  // - Si l'action ne lance pas de lecture (★ favoris), on restaure la diffusion en cours.
  const iconBtn = t.closest?.('button.icon-btn');
  const iconText = (iconBtn?.textContent || '').trim();
  const clickedChannelItem = !!t.closest?.('.channel-item');

  if (iconText === '★') {
    stopRadioAndRestore();
  } else if (clickedChannelItem) {
    stopRadioAndExitOverlayOnly();
  } else {
    // fallback safe
    stopRadioAndRestore();
  }
...
// Capture = on ferme l'overlay AVANT les handlers des listes, sans casser leurs actions
document.addEventListener('click', exitRadioOverlayOnSidebarClick, true);
